...
# ByPy 初始化参数配置
# ============================================================
# access_token: 访问令牌（可选，未提供时会引导认证）
# slice_size: 分片大小（字节），默认 20MB，大文件可设为 50MB
# concurrency: 并发任务数，默认 5，可根据网络和系统性能调整
# ondup: 重复文件处理方式，'overwrite'(覆盖) / 'skip'(跳过) / 'rename'(重命名)
BYPY_CONFIG = {
    'slice_size': 50 * 1024 * 1024,  # 20MB
}

bp = ByPy(
    slice_size=BYPY_CONFIG['slice_size'],
)
...
for idx, (local, remote) in enumerate(exist):
    if os.path.isdir(local):
        ...
        # 2. 上传
        bp.upload(zip_path, remote_zip, ondup='skip')
        # 3. 立即清理
        ...
        bp.upload(local, remote, ondup='skip')
