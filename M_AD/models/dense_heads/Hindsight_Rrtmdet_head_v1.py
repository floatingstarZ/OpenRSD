...
"""
基于Gen_Rrtmdet_head_v8_ICTHead
对Attention进行了改进，引入mask机制，避免padding带来的影响
统一规定：
>= 0 为正样本
-1代表padding
<= -2 为负样本
...
class InstanceAlignmentEmbed(nn.Module):
    ...
    def forward(self,
        ...
        sims = q_ @ k_.transpose(-1, -2)
        temperature = torch.Tensor([0.07]).to(q_.device)
        matching_scores = sims / temperature

        # ----- 每个类别取最大，-1、-2这种负类别设置为最后一个类，
        N, M = matching_scores.shape
        num_classes = kwargs['num_classes']
        valid_support_labels = deepcopy(k_label)
        valid_support_labels[valid_support_labels < 0] = num_classes
        valid_support_labels = valid_support_labels[None, :].expand([N, M])

        # ----- 如果valid_support_labels有-1类，那么cls_scores的最后一列有数值，否则为-100
...
class BaseClsHead(nn.Module):
    ...
    def get_matching_scores(self,
        ...
        matching_scores[support_labels < 0] = -100
        """
        # ----- 设置与support_labels < 0 之间的相似度为-100，不计入损失
        # ----- support_labels < 0 无法用来进行scatter_reduce，由于support_labels实际上代表了index，因此将support_labels < 0 映射为num_classes
        valid_support_labels = deepcopy(support_labels)
        valid_support_labels[valid_support_labels < 0] = num_classes
        ...
        cls_scores = cls_scores.scatter_reduce(-1, valid_support_labels.long(), matching_scores, reduce="amax")
        # neg_cls_scores = cls_scores[..., -1]
        # print(torch.max(neg_cls_scores))
...
from M_AD.models.utils.transformer_modular_yolo_with_mask import TwoWayTransformerModularYOLO
...
class OpenRotatedRTMDetSepBNHead(RotatedRTMDetSepBNHead):
        ...
        if return_predict:
            if pred_cfg is None:
                pred_cfg = dict(
                        ...
                        max_per_img=2000)
            pred_cfg = ConfigDict(pred_cfg)
            pred_inputs = outs[:3] # cls_scores, bbox_preds, angle_preds
    ...
    def forward(self,
        ...
        # ---- Cross Attention 中 padding的特征不参与
        support_masks = (support_labels != -1).int()
        # print('PRINT_support_labels', support_labels)
        # print('PRINT_support_masks', support_masks)
        out_support_feats, out_flatten_feats = self.cross_attention(queries=support_feats,
                                                                    keys=in_flatten_feats,
                                                                    query_masks=support_masks,
                                                                    key_masks=None)
    ...
    def loss_by_feat(self,
        ...
        if self.with_obj_align:
            ...
            accs = torch.Tensor([0.0]).to(device)
            ...
            for i in range(num_imgs):
                ...
                if len(pos_labels) == 0 or len(s_labels) == 0:
                    loss_align = loss_align + torch.Tensor([0.]).to(device)
                    accs = accs + torch.Tensor([0.]).to(device)
                    continue

                # ----- 计算对齐损失
                pos_cls_scores = self.obj_align_branch(pos_pred_embeds,
                                                   s_feats,
                                                   pos_labels,
                                                   s_labels,
                                                   visual_fc=self.visual_fc,
                                                       text_fc=self.text_fc,
                                                   **kwargs)
                loss_align_ = F.cross_entropy(pos_cls_scores, pos_labels,
                                              reduction='mean')
                # print('pos_cls_scores', pos_cls_scores)
                # print('pos_labels', pos_labels)
                # print('loss_align_', loss_align_)

                predicted = torch.argmax(pos_cls_scores, dim=1)
                correct = (predicted == pos_labels).sum()
                accuracy = correct / max(pos_labels.size(0), 1) * 100

                loss_align = loss_align + loss_align_
                accs = accs + accuracy.detach()

            loss_align = loss_align / num_imgs
            align_acc = accs / num_imgs
            losses['loss_aln'] = loss_align
            losses['align_acc'] = align_acc.detach()
        ##############################################################################
